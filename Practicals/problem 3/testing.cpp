...
{
    ifstream fin;
    fin.open("Backup.csv",ios::in);
    if (!fin)
    {
        cout << "No records file present!" << endl;
        // exit(1);
    ...
    while (!fin.eof())
    {
        ...
        prt = (upload_Data(Back));
        // if ((last1 == prt->parts_id && last2 == prt->company_id) || (prt->company_id == "") || (prt->parts_id == ""))
        //     break;
        Insert(prt);
        // last1 = prt->parts_id;
